...
@onready var left_arm : Polygon2D = $"Body/Left Arm"
@onready var left_lower_arm : Polygon2D = $"Body/Left Arm/Left Lower Arm"
@onready var right_arm : Polygon2D = $"Body/Right Arm"
...
@onready var limbs : Array[Polygon2D] = [left_arm, right_arm, left_leg, right_leg]
@onready var lower_limbs : Array[Polygon2D] = [left_lower_arm, right_lower_arm, left_calf, right_calf]


func _process(delta):
	for limb : Polygon2D in limbs:
		limb.transform = limb.transform.rotated_local(delta)
	for lower_limb : Polygon2D in lower_limbs:
